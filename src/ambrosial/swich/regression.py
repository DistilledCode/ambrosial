...
class RegressionPlot:
    ...
    def order_amount(self, bins: str) -> None:
    ...
    def ordamt_ordfeeprcnt(self, bins: str, remove_outliers: bool = True) -> None:
        ...
        p.set_xticks([int(i) for i in np.linspace(min(df["x"]), max(df["x"]), 20)])
        p.set_yticks([float(i) for i in np.linspace(min(df["y"]), max(df["y"]), 20)])
        p.set_ylabel("Fee as % of Order Total\n", fontsize=15)
        p.set_xlabel("Order Total    (in ₹)", fontsize=15)

    def ordamt_ordfee(self, bins: str, remove_outliers: bool = True) -> None:
        ...
        p.set_xticks([int(i) for i in np.linspace(min(df["x"]), max(df["x"]), 20)])
        p.set_yticks([int(i) for i in np.linspace(min(df["y"]), max(df["y"]), 20)])
